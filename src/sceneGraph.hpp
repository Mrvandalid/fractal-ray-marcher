...
enum SceneNodeType {
	GEOMETRY_3D, GEOMETRY_3D_NORMAL_MAP, GEOMETRY_2D, POINT_LIGHT, SPOT_LIGHT
};
...
struct SceneNode {
	...
	// The identification number if this node is a light.
	int lightIndex;

	// The ID of the Texture, if this node contains a texture.
	int mainTextureID;

	// The ID of the normal map, if the model has it.
	int normalMapID;

	// The ID of the normal map, if the node contains it.
	int roughnessMapID;
